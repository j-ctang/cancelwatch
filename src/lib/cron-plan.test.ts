...
describe("planCronActions", () => {
  test("sends a 3-day reminder when the deadline is 3 days out", () => {
    const plan = planCronActions([row({ nextDeadline: "2026-01-13" })], TODAY);

    expect(plan.reminders).toEqual([{ id: "row-1", kind: "3-day" }]);
    expect(plan.rollovers).toEqual([]);
  });

  test("sends a 1-day reminder when the deadline is 1 day out", () => {
    const plan = planCronActions([row({ nextDeadline: "2026-01-11" })], TODAY);

    expect(plan.reminders).toEqual([{ id: "row-1", kind: "1-day" }]);
  });
  ...
  test("handles multiple rows independently", () => {
    const plan = planCronActions(
      [
        row({ id: "a", nextDeadline: "2026-01-13" }),
        row({ id: "b", nextDeadline: "2026-01-05", cycleDays: 30 }),
        row({ id: "c", nextDeadline: "2026-01-15" }),
      ],
      TODAY
    );

    expect(plan.reminders).toEqual([{ id: "a", kind: "3-day" }]);
